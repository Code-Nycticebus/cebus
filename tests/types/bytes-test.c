...
static void test_bytes(void) {
  ...
  cebus_assert(b.data[0] == 0x02, "Bytes are not initialized correctly");
  cebus_assert(b.data[1] == 0xff, "Bytes are not initialized correctly");
  cebus_assert(b.data[2] == 0xaa, "Bytes are not initialized correctly");
  cebus_assert(b.data[3] == 0xbb, "Bytes are not initialized correctly");
  arena_free(&arena);
}

static void test_bytes_str(void) {
  Bytes b = BYTES_STR("ABC");
  cebus_assert(b.size == 3, "Size is not correct");
  cebus_assert(b.data[0] == 0x41, "Bytes are not initialized correctly");
  cebus_assert(b.data[1] == 0x42, "Bytes are not initialized correctly");
  cebus_assert(b.data[2] == 0x43, "Bytes are not initialized correctly");

  Bytes b2 = BYTES_STR("\x41\x42\x43");
  cebus_assert(b2.size == 3, "Size is not correct");
  cebus_assert(b2.data[0] == 0x41, "Bytes are not initialized correctly");
  cebus_assert(b2.data[1] == 0x42, "Bytes are not initialized correctly");
  cebus_assert(b2.data[2] == 0x43, "Bytes are not initialized correctly");
}
...
static void test_to_hex(void) {
  ...
  cebus_assert(str_eq(s, STR("414243")), "String conversion was not correct: '" STR_FMT "'",
               STR_ARG(s));
  ...
  Bytes b3 = BYTES(0xaa, 0xbb, 0xcc, 0xdd, 0x41, 0x41, 0x41, 0x41, 0x42, 0x42, 0x42, 0x42);
...
static void test_bytes_cmp(void) {
  Bytes b1 = BYTES(0xFF);
  cebus_assert(bytes_eq(b1, BYTES(0xFF)) == true, "Bytes not compared correctly");
  cebus_assert(bytes_eq(b1, BYTES(0x0F)) == false, "Bytes not compared correctly");
}

static void test_bytes_slice(void) {
  Bytes b = BYTES_STR("ABC");
  Bytes slice = bytes_slice(b, 1, 3);
  cebus_assert(bytes_eq(slice, BYTES_STR("BC")), "Bytes were not sliced correctly");
  ...
  cebus_assert(bytes_eq(invalid2, BYTES_STR("")), "Bytes were not invalidated");
}
...
static void test_bytes_take(void) {
  ...
  cebus_assert(bytes_eq(take, BYTES(0x12, 0x34, 0x56)), "Did not take the correct bytes");
  ...
  cebus_assert(invalid.size == 0, "Did not take the correct amount of bytes");
}
...
static void test_bytes_hash(void) {
  ...
  cebus_assert(bytes_hash(b1) == bytes_hash(b2), "should be equal");
  cebus_assert(bytes_hash(b1) != bytes_hash(b3), "should not be equal");
}
