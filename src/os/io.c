...
void io_write(FILE *file, Bytes bytes, Error *error) {
  ...
  if (ferror(file)) {
    error_init(error, errno, "Could not write file: %s", strerror(errno));
  }
}
...
Bytes io_read(FILE *file, usize size, u8 buffer[size], Error *error) {
  ...
  if (ferror(file)) {
    error_init(error, errno, "Could not read file: %s", strerror(errno));
    return (Bytes){0};
  }
...
Str io_read_line(FILE *file, usize size, char buffer[size], Error *error) {
  ...
  if (ferror(file)) {
    error_init(error, errno, "Could not read line: %s", strerror(errno));
    return (Str){0};
  }
